        ...
        const drawCanvas = () => {
            ...
            for (let i = 0; i < hexagonSide; i += 1) {
                ...
                context.fillText(
                    texts[i].text,
                    hexagonPosX +
                        (hexagonRadius + 50) * Math.sin(hexagonAngle * i),
                    hexagonPosY +
                        (hexagonRadius + 50) * Math.cos(hexagonAngle * i)
                );
            }
            ...
            for (let k = 0; k < score.current.length; k += 1) {
                const hexagonPoints = Array.from({
                    ...
                    x:
                        ...
                        hexagonRadius *
                            score.current[k][i] *
                            Math.sin(hexagonAngle * i),
                    y:
                        hexagonPosY +
                        hexagonRadius *
                            score.current[k][i] *
                            Math.cos(hexagonAngle * i),
                }));
